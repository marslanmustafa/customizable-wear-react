...
const ProductTypePage = () => {
	...
	// 2. Improve the filtering useEffect
	useEffect(() => {
		...
		let results = products.filter((product) => {
			// First, ensure productType exists and is an array
			if (!Array.isArray(product.productType)) {
				console.warn('Product missing productType array:', product._id);
				return false;
			}

			// Case-insensitive comparison
			const matches = product.productType.some((type) => type.toLowerCase() === productType.toLowerCase());
	...
	}, [products, productType, priceRange, selectedRatings, sortOption]);
							...
							{expandedFilters.category && (
								<div className='mt-4 space-y-2'>
									{allCategories.map((category, index) => (
										<button
											key={index}
											className={`flex items-center w-full text-left px-3 py-2 rounded-md text-sm transition-colors ${productType.toLowerCase() === category.toLowerCase()
