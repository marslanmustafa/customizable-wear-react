...
import { loginSuccess, logout } from '../store/authSlice'; // Make sure you have logout action
...
const LoginPage = () => {
	...
	const onSubmit = async (data) => {
		try {
			...
			if (response.ok && result.success) {
				// Check if user is seller before proceeding
	...
	const handlePostLoginRedirect = (user) => {
		const { from, openPopup } = location.state || {};

		// Double check user role (in case the first check was bypassed)
		if (user.role === 'seller') {
			dispatch(logout());
			toast({
				title: 'Access Denied',
				description: 'Sellers must login through the seller portal',
				variant: 'destructive',
				className: 'bg-red-500 text-white border-0',
			});
			return;
		...
		// Priority 1: Redirect back to original protected page if exists
		if (from) {
		...
		<div className='flex items-center justify-center min-h-screen bg-gray-100 px-4 md:px-0'>
			<div className='w-full max-w-md bg-white rounded-lg shadow-lg p-8 relative'>
				...
				<div className='mt-4 text-center'>
					...
					<button onClick={() => navigate('/signup')} className='ml-2 text-blue-500 hover:underline focus:outline-none'>
						Sign up
					</button>
				</div>
